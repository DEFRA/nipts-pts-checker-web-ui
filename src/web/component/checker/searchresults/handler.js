...
const buildCheckOutcome = (request, data, checklist) => {
...
const handleServiceError = (request, h, checklist) => {
...
const cleanupAndSetSuccess = (request) => {
...
const saveAndContinueHandler = async (request, h) => {
  try {
    ...
    if (checklist === CheckOutcomeConstants.Pass) {
      const checkOutcome = buildCheckOutcome(request, data, checklist);
      ...
      if (responseData?.error) {
        return handleServiceError(request, h, checklist);
      }

      cleanupAndSetSuccess(request);
